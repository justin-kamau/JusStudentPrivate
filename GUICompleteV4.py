...
class MathGame(Frame):
    ...
    def ParentsReports(self,WhichFrame):
        ...
        print(MWrong)
        TWrong = MWrong + AWrong +  SWrong + DWrong
        print(TWrong)
        print(count)
        ACorrect = count - TWrong
        FirstPercentage =( ACorrect/count ) * 100
        FirstPercentage = round(FirstPercentage)
    
        if FirstPercentage >= 50:
            colour = "Green"
        elif FirstPercentage <50:
            colour = "Red"
        ReportFile.close()
        lblQAnswered = Label(self.ParentsReport,text="Your child has answered a total of " +str(count)+" Questions",font=["Arial",15],bg="white" )
        lblCorrectRatio = Label(self.ParentsReport,text= "Their overall percentage: " + str(FirstPercentage)+"%",font=["Arial",18],bg=colour)
        lblOWrong = Label(self.ParentsReport,text=str(ChildsName) + " has got the following questions wrong: ",font=["Arial",15],bg="white") 
        ...
        lblCorrectRatio.grid(row=6,column=0)
        lblOWrong.grid(row=2,column = 0)
        lblWrong.grid(row = 3,column = 0)
        lblDirect.grid(row=8,column = 0)

        btnMultiplication.grid(row=9,column=0)
    ...
    def DetailedReportScreen(self,WhichFrame,IncorrectList):
        ...
        for Item in IncorrectList:
            ...
            i = i + 1
        lblresult = ""
        btnBack = Button(self.DetailedReportScreenFrame,text="Back",command=lambda:self.goToMenu(lblresult))
        btnBack.grid(row=2,column=2)
        lblTitle.grid(row=0,column=0)
